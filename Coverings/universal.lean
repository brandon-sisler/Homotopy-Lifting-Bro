...
def temp (X: Type _)[TopologicalSpace X] ( x₀ : X ) (U : Set X) (h : U ∈ slsc_pc_nbhds X) ( γ : Path x₀ (get_point U (slsc_nbhd_is_nonempty X h ))) (u : U) : UniversalCover X x₀ :=
...
def all_local_compositions (X: Type _)[TopologicalSpace X] ( x₀ : X ) (U : Set X) (h : U ∈ slsc_pc_nbhds X) ( γ : Path x₀ (get_point U (slsc_nbhd_is_nonempty X h ))): Set ( UniversalCover X x₀ ) := 
  temp X x₀ U h γ '' Set.univ 

-- #check fun p : Σ (U : Set X), Path x₀ (get_point U (slsc_nbhd_is_nonempty X _ ) ) ↦  all_local_compositions X x₀ p.1 p.2 
-- #check fun p : ((A : Prop) × (B : Prop)) × (C : Prop) ↦ all_local_compositions 

#check Σ (a : ℕ) (b : ℕ), Unit
...
def lifts_of_slsc_pc_nbhds (X : Type _) [TopologicalSpace X ] (x₀ : X) 
 ...
 -- Original version suggested by Patrick, Doesn't work because it was necessary to 
 -- pass an argument that U belongs to the sollection of sets generated by slsc_pc_nbds X,
 -- this is NOT THE SAME as declaring U : slsc_pc_nbhds

 -- not clear how to pass the assumption h : U ∈ slsc_pc_nbhds X
  have := range fun p : ( Σ U : {U' : Set X // U' ∈ slsc_pc_nbhds X} , Path x₀ (get_point U.1 (slsc_nbhd_is_nonempty X U.2 )) ) ↦  all_local_compositions X x₀ p.1.1 p.1.2 p.2  --all_local_compositions X x₀ p.1 _ --p.2
  sorry
 -- Attempts to rework it
 -- range (fun p : ( Σ (U : Set X), Path x₀ (get_point U (slsc_nbhd_is_nonempty X _ ) ) ) ↦  all_local_compositions X x₀ p.1 p.2)


--#check (fun p₁ : (U : Set X) × (U ∈ slsc_pc_nbhds X) ↦ ( Path x₀ (get_point U (slsc_nbhd_is_nonempty X h ) ) ) )
